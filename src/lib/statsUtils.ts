...
export const processActivityFrequency = (activities: PuppyActivity[]) => {
...
export const processActivityTrend = (activities: PuppyActivity[], anchorDate: Date = new Date()) => {
...
export const processActivityDuration = (activities: PuppyActivity[]) => {
  const durationStats = activities.reduce((acc, activity) => {
    if (activity.end_time) {
      ...
      if (minutes > 0 && minutes < 1440) { // filter out absurd durations
